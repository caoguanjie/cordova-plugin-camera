...
#pragma clang diagnostic pop
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[[UIAlertView alloc] initWithTitle:[[NSBundle mainBundle]
                                                ...
                                                message:NSLocalizedString(@"禁止进入相机；请在设置应用程序中启用它.", nil)
                                               delegate:weakSelf
...
#pragma clang diagnostic pop
    ...
    CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"无法进入相机"];   // error callback expects string ATM
...
- (void)cleanup:(CDVInvokedUrlCommand*)command
{
    ...
    if (hasErrors) {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_IO_EXCEPTION messageAsString:@"删除一个或多个文件失败."];
    } else {
...
- (void)popoverControllerDidDismissPopover:(id)popoverController
{
    ...
    if (self.pickerController && self.pickerController.callbackId && self.pickerController.pickerPopoverController) {
        ...
        CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"没有选择图片"];   // error callback expects string ATM
...
- (void)imagePickerControllerDidCancel:(UIImagePickerController*)picker
{
    ...
    dispatch_block_t invoke = ^ (void) {
        ...
        if (picker.sourceType == UIImagePickerControllerSourceTypeCamera && [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo] != ALAuthorizationStatusAuthorized) {
            result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"无法进入相机"];
            ...
            result = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"没有选择图片"];
        }
